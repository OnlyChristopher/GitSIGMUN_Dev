...
class tesocajareccontriopController extends Zend_Controller_Action
{
...
public function registrapagoAction(){
	...
	//			echo "<h2>$key</h2>";
			...
			$nombrestore="Rentas.sp_Genera_OP";
			...
			if(count($rowrecibos))
			{
				foreach($rowrecibos AS $row){
				$entry = array(
					'idvalor'=>trim($rowrecibos[0][0]),
					'numval'=>trim($rowrecibos[0][1]),
					'anoval'=>trim($rowrecibos[0][2])
					);
				$jsonData[] = $entry;
				}
			}
