...
class MantinfractorController extends Zend_Controller_Action
{
    ...
    {    		
		...
		$evt[] = array('#cmbTipoContri2',"change","suceciondetalle($(this).val());");
		...
		if(strlen($codigo)>0)
		{
			...
			$rowContri = $cn->ejec_store_procedura_sql('[Mpreventiva].[sp_Infractor]', $parametros);
			...
			$correo=$rowContri[0][52];
			$partida_defuncion=$rowContri[0][53];
			$fecha_defuncion=$rowContri[0][54];
			$telefono1=trim($rowContri[0][55]);
			$anexo1=trim($rowContri[0][56]);
			$telefono2=trim($rowContri[0][57]);
			$anexo2=trim($rowContri[0][58]);
			$flag_notificacion=trim($rowContri[0][59]);
			$this->view->action = 'E';
		...
		$val[] = array('#txt_correo',$correo,'val');
		$val[] = array('#txtdefuncion',$partida_defuncion,'val');
		$val[] = array('#txtfec_defuncion',$fecha_defuncion,'val');
		
		$val[] = array('#txt_telefono1',$telefono1,'val');
		$val[] = array('#txt_anexo1',$anexo1,'val');
		$val[] = array('#txt_telefono2',$telefono2,'val');
		$val[] = array('#txt_anexo2',$anexo2,'val');
		...
		*/
		...
		$evt[] = array('#btnGrabaContri',"click","goToFormulario('frminfractor');");
		$evt[] = array('#btnSalirContri',"click","closePopup('#popinfractor');");
		...
		$evt[] = array('#txtfec_defuncion',"datepicker","");
		
		if($flag_notificacion=="1"){
			$cad[] = array('#ckNotificacion',"checked", 'true');
		}
    ...
    public function grabarAction()
    {
    	...
    	if($this->getRequest()->isXmlHttpRequest()){
			...
			$parametros[] = array('@manzana',str_replace("'","''",$this->_request->getPost('txtMz')));
			...
			$parametros[] =array('@correo_e',$this->_request->getPost('txt_correo'));
			$parametros[] =array('@partida_defuncion',$this->_request->getPost('txtdefuncion'));
			$parametros[] =array('@fecha_defuncion',$this->_request->getPost('txtfec_defuncion'));
			
			$parametros[] =array('@telefono1',$this->_request->getPost('txt_telefono1'));
			$parametros[] =array('@anexo1',$this->_request->getPost('txt_anexo1'));
			$parametros[] =array('@telefono2',$this->_request->getPost('txt_telefono2'));
			$parametros[] =array('@anexo2',$this->_request->getPost('txt_anexo2'));
			
			$parametros[] =array('@flag_notificar',$this->_request->getPost('ckNotificacion'));
