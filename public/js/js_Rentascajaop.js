Ext.onReady(function(){
	...
	*/
    ...
    var store = Ext.create('Ext.data.Store', {
        ...
        proxy: {
            type: 'ajax',
            //url : 'rentascajareccontriop/conreccontri?codigo='+codigocaja,
			url : 'rentascajareccontriop/conreccontri',
            reader: {
               type: 'json',
               root: 'rows'
            }, 
...
function mostrarRecContri(){
	...
	*/
	...
	var fechaproyec=$('#txtFechaproyeccion').val();
...
function generarop(){
	...
	var divFechaproyec = $('#txtFechaproyeccion').val();
		...
		if(selectedRecordsArray.length>0){
		 ...
		 var showResult = function(btn){
			if(btn=='yes'){
				...
				Ext.Ajax.request({
		            url: urljs + "tesocajareccontriop/registrapago",
		            method: "POST",
 		...
 		confirmMessage('Contribuyentes','Seguro de Generar OP?',showResult);
