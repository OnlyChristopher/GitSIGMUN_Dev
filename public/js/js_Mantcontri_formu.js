...
$(function(){	
	...
	$('#frmcontri').validate({
		...
		submitHandler: function(form){
			...
			if (array[0]=='false')
			{
				...
				$.ajax({     
					type: "POST",  
					url: urljs + "mantcontri/grabar",
					data: $('#frmcontri').serializeObject(),     
					success: function(data) { 
						//infoMessage('Guardar Contribuyente',data);
						if(data=='A'){
							closePopup('#popcontri');
							Ext.getCmp('xgridContri').getStore().load();
						}
						if(data=='B'){
	...
	if($('#cmbTipoContri2').val()=='02')
	{
		//alert($('#suceciondetalle').val());
