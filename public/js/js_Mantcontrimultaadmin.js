Ext.onReady(function(){
    ...
    var store = Ext.create('Ext.data.Store', {
        ...
        proxy: {
            type: 'ajax',
            url : 'Mantcontrimultaadmin/consulta',
            reader: {
               type: 'json',
               root: 'rows'
            }
    ...
    var grid = Ext.create('Ext.grid.Panel', {
        ...
        Ext.create('Ext.grid.RowNumberer'),    
        {
        	...
        	text: 'Apellidos Nombres/Razon Social',
        	width: 380,
            dataIndex: 'nombres'
        },{
            text: 'Predios',
            ...
            width:82,
            items: [{
                icon: urljs + 'img/lupa.png',
                tooltip: 'Multas Administrativas ',
                handler: function(grid, rowIndex, colIndex) {
                    var rec = grid.getStore().getAt(rowIndex);
                    //Ext.MessageBox.alert('Editar',rec.get('codigo'));
                    showPopup('Multasadmin/index?codigo='+rec.get('codigo'),'#popcontri','820','470','Multas Administrativas');
                },
                ...
                handler: function(grid, rowIndex, colIndex) {
                    var rec = grid.getStore().getAt(rowIndex);
                    //Ext.MessageBox.alert('Editar',rec.get('codigo'));
                    showPopup('mantcontri/formu?codigo='+rec.get('codigo'),'#popcontri','820','470','Editar Contribuyente');
                },
                getClass: function(value,metadata,record){
                	if (mod=='14')
                		return gridColumnAction(false);
