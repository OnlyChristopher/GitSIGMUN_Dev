...
function generasimulado(){
	...
	if(gridCuotalval.getStore().data.length > 0){
		...
		if($("#txtPorcentaje").val().replace(',','')< $('#porc_ini').val()){
			var msg='El porcentage no puede ser menos del '+$('#porc_ini').val()+'%';
			infoMessage('Fraccionamiento',msg);
		}
		else{
...
function generaconvenio(){
	...
	var condicon_id=$('#condicion_id').val();
	...
	if(gridCuotalval.getStore().data.length > 0){
		...
		if(selectedRecordsArray.length>0){
		if($("#txtPorcentaje").val().replace(',','')< $('#porc_ini').val()){
			var msg1='El porcentage no puede ser menos del '+$('#porc_ini').val()+'%';
			infoMessage('Fraccionamiento',msg1);
		}
		else{
			 var showResult = function(btn){
				if(btn=='yes'){
					...
					Ext.Ajax.request({
			            ...
			            method: "POST",
			            params: {json: JSON.stringify(idList),cuotas: cuotas,total_deuda:total_deuda,total_inici:total_inici,total_inici:total_inici,fec_gen:fec_gen,fec_cuo:fec_cuo,condicon_id:condicon_id},
...
function mostrarcuotas(max_cuotas){
	...
	if((Math.round(total_inici*100)/100) > (Math.round(total_deuda*100)/100) ){
		...
		if($('#txtNumero').val()>0 && $('#txtNumero').val()<=max_cuotas)
		{	
		...
		{
			infoMessage('SIGMUN','La cantidad de cuotas no esta permitida');
			$('#txtNumero').val('0');
		}
...
{	
	...
	var porc_establecido=$("#porc_ini").val();

	if(Number($("#txtPorcentaje").val().replace(',',''))< porc_establecido ){
		var mensaje='El porcentage no puede ser menos del '+porc_establecido+'%';
		infoMessage('Fraccionamiento',mensaje);
		$("#txtInicial").val(0);
		return;
	}
